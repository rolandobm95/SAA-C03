# AWS Certified Solutions Architect – Associate (SAA-C03)

# Servicios de AWS dentro del alcance del examen

> Lista organizada por categorías según la guía del examen SAA-C03.  
> Cada servicio incluye enlace directo a la documentación oficial de AWS.

**Navegación:** [Inicio](../README.md) · [Anterior: Preguntas - Dominio 4](../SAA-C03-Questions/SAA-C03-Questions-D4.md)

---

# Índice

- [Análisis](#análisis)
- [Integración de aplicaciones](#integración-de-aplicaciones)
- [Administración de costos de AWS](#administración-de-costos-de-aws)
- [Computación](#computación)
- [Contenedores](#contenedores)
- [Base de datos](#base-de-datos)
- [Herramientas para desarrolladores](#herramientas-para-desarrolladores)
- [Frontend web y móvil](#frontend-web-y-móvil)
- [Machine Learning](#machine-learning)
- [Administración y gobernanza](#administración-y-gobernanza)
- [Servicios multimedia](#servicios-multimedia)
- [Migración y transferencia](#migración-y-transferencia)
- [Redes y entrega de contenido](#redes-y-entrega-de-contenido)
- [Seguridad, identidad y cumplimiento](#seguridad-identidad-y-cumplimiento)
- [Sin servidor](#sin-servidor)
- [Almacenamiento](#almacenamiento)
- [Recursos oficiales del examen](#recursos-oficiales-del-examen)

---

# Análisis

- [Amazon Athena](https://docs.aws.amazon.com/athena/)
- [AWS Data Exchange](https://docs.aws.amazon.com/data-exchange/)
- [Amazon Data Firehose](https://docs.aws.amazon.com/firehose/)
- [Amazon EMR](https://docs.aws.amazon.com/emr/)
- [AWS Glue](https://docs.aws.amazon.com/glue/)
- [Amazon Kinesis](https://docs.aws.amazon.com/kinesis/)
- [AWS Lake Formation](https://docs.aws.amazon.com/lake-formation/)
- [Amazon Managed Streaming for Apache Kafka (Amazon MSK)](https://docs.aws.amazon.com/msk/)
- [Amazon OpenSearch Service](https://docs.aws.amazon.com/opensearch-service/)
- [Amazon Quick](https://docs.aws.amazon.com/quick/)
- [Amazon Redshift](https://docs.aws.amazon.com/redshift/)

---

# Integración de aplicaciones

- [Amazon AppFlow](https://docs.aws.amazon.com/appflow/)
- [AWS AppSync](https://docs.aws.amazon.com/appsync/)
- [Amazon EventBridge](https://docs.aws.amazon.com/eventbridge/)
- [Amazon MQ](https://docs.aws.amazon.com/amazon-mq/)
- [Amazon SNS](https://docs.aws.amazon.com/sns/)
- [Amazon SQS](https://docs.aws.amazon.com/sqs/)
- [AWS Step Functions](https://docs.aws.amazon.com/step-functions/)

---

# Administración de costos de AWS

- [AWS Budgets](https://docs.aws.amazon.com/cost-management/latest/userguide/budgets-managing-costs.html)
- [AWS Cost and Usage Report](https://docs.aws.amazon.com/cur/)
- [AWS Cost Explorer](https://docs.aws.amazon.com/cost-management/latest/userguide/ce-what-is.html)
- [Savings Plans](https://docs.aws.amazon.com/savingsplans/)

---

# Computación

- [AWS Batch](https://docs.aws.amazon.com/batch/)
- [Amazon EC2](https://docs.aws.amazon.com/ec2/)
- [Amazon EC2 Auto Scaling](https://docs.aws.amazon.com/autoscaling/ec2/)
- [AWS Elastic Beanstalk](https://docs.aws.amazon.com/elasticbeanstalk/)
- [AWS Outposts](https://docs.aws.amazon.com/outposts/)
- [AWS Serverless Application Repository](https://docs.aws.amazon.com/serverlessrepo/)
- [VMware Cloud on AWS](https://docs.aws.amazon.com/vmware-cloud-on-aws/)
- [AWS Wavelength](https://docs.aws.amazon.com/wavelength/)

---

# Contenedores

- [Amazon ECR](https://docs.aws.amazon.com/ecr/)
- [Amazon ECS](https://docs.aws.amazon.com/ecs/)
- [Amazon ECS Anywhere](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-anywhere.html)
- [Amazon EKS](https://docs.aws.amazon.com/eks/)
- [Amazon EKS Anywhere](https://docs.aws.amazon.com/eks-anywhere/)
- [Amazon EKS Distro](https://docs.aws.amazon.com/eks/latest/userguide/eks-distro.html)

---

# Base de datos

- [Amazon Aurora](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/)
- [Amazon Aurora Serverless](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-serverless.html)
- [Amazon DocumentDB](https://docs.aws.amazon.com/documentdb/)
- [Amazon DynamoDB](https://docs.aws.amazon.com/dynamodb/)
- [Amazon ElastiCache](https://docs.aws.amazon.com/elasticache/)
- [Amazon Keyspaces](https://docs.aws.amazon.com/keyspaces/)
- [Amazon Neptune](https://docs.aws.amazon.com/neptune/)
- [Amazon RDS](https://docs.aws.amazon.com/rds/)
- [Amazon Redshift](https://docs.aws.amazon.com/redshift/)

---

# Herramientas para desarrolladores

- [AWS X-Ray](https://docs.aws.amazon.com/xray/)

---

# Frontend web y móvil

- [AWS Amplify](https://docs.aws.amazon.com/amplify/)
- [Amazon API Gateway](https://docs.aws.amazon.com/apigateway/)
- [AWS Device Farm](https://docs.aws.amazon.com/devicefarm/)

---

# Machine Learning

- [Amazon Comprehend](https://docs.aws.amazon.com/comprehend/)
- [Amazon Kendra](https://docs.aws.amazon.com/kendra/)
- [Amazon Lex](https://docs.aws.amazon.com/lex/)
- [Amazon Polly](https://docs.aws.amazon.com/polly/)
- [Amazon Rekognition](https://docs.aws.amazon.com/rekognition/)
- [Amazon SageMaker AI](https://docs.aws.amazon.com/sagemaker/)
- [Amazon Textract](https://docs.aws.amazon.com/textract/)
- [Amazon Transcribe](https://docs.aws.amazon.com/transcribe/)
- [Amazon Translate](https://docs.aws.amazon.com/translate/)

---

# Administración y gobernanza

- [AWS Auto Scaling](https://docs.aws.amazon.com/autoscaling/)
- [AWS CLI](https://docs.aws.amazon.com/cli/)
- [AWS CloudFormation](https://docs.aws.amazon.com/cloudformation/)
- [AWS CloudTrail](https://docs.aws.amazon.com/awscloudtrail/)
- [Amazon CloudWatch](https://docs.aws.amazon.com/cloudwatch/)
- [AWS Compute Optimizer](https://docs.aws.amazon.com/compute-optimizer/)
- [AWS Config](https://docs.aws.amazon.com/config/)
- [AWS Control Tower](https://docs.aws.amazon.com/controltower/)
- [AWS Health Dashboard](https://docs.aws.amazon.com/health/)
- [AWS License Manager](https://docs.aws.amazon.com/license-manager/)
- [Amazon Managed Grafana](https://docs.aws.amazon.com/grafana/)
- [Amazon Managed Service for Prometheus](https://docs.aws.amazon.com/prometheus/)
- [AWS Management Console](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/getting-started.html)
- [AWS Organizations](https://docs.aws.amazon.com/organizations/)
- [AWS Service Catalog](https://docs.aws.amazon.com/servicecatalog/)
- [AWS Systems Manager](https://docs.aws.amazon.com/systems-manager/)
- [AWS Trusted Advisor](https://docs.aws.amazon.com/awssupport/latest/user/trusted-advisor.html)
- [AWS Well-Architected Tool](https://docs.aws.amazon.com/wellarchitected/latest/userguide/intro.html)

---

# Servicios multimedia

- [Amazon Elastic Transcoder](https://docs.aws.amazon.com/elastictranscoder/)
- [Amazon Kinesis Video Streams](https://docs.aws.amazon.com/kinesisvideostreams/)

---

# Migración y transferencia

- [AWS Application Migration Service](https://docs.aws.amazon.com/mgn/)
- [AWS DataSync](https://docs.aws.amazon.com/datasync/)
- [AWS Database Migration Service (AWS DMS)](https://docs.aws.amazon.com/dms/)
- [AWS Snow Family](https://docs.aws.amazon.com/snowball/)
- [AWS Transfer Family](https://docs.aws.amazon.com/transfer/)

---

# Redes y entrega de contenido

- [AWS Client VPN](https://docs.aws.amazon.com/vpn/latest/clientvpn-admin/)
- [Amazon CloudFront](https://docs.aws.amazon.com/cloudfront/)
- [AWS Direct Connect](https://docs.aws.amazon.com/directconnect/)
- [Elastic Load Balancing (ELB)](https://docs.aws.amazon.com/elasticloadbalancing/)
- [AWS Global Accelerator](https://docs.aws.amazon.com/global-accelerator/)
- [AWS PrivateLink](https://docs.aws.amazon.com/vpc/latest/privatelink/)
- [Amazon Route 53](https://docs.aws.amazon.com/route53/)
- [AWS Site-to-Site VPN](https://docs.aws.amazon.com/vpn/latest/s2svpn/)
- [AWS Transit Gateway](https://docs.aws.amazon.com/vpc/latest/tgw/)
- [Amazon VPC](https://docs.aws.amazon.com/vpc/)

---

# Seguridad, identidad y cumplimiento

- [AWS Artifact](https://docs.aws.amazon.com/artifact/)
- [AWS Audit Manager](https://docs.aws.amazon.com/audit-manager/)
- [AWS Certificate Manager (ACM)](https://docs.aws.amazon.com/acm/)
- [AWS CloudHSM](https://docs.aws.amazon.com/cloudhsm/)
- [Amazon Cognito](https://docs.aws.amazon.com/cognito/)
- [Amazon Detective](https://docs.aws.amazon.com/detective/)
- [AWS Directory Service](https://docs.aws.amazon.com/directoryservice/)
- [AWS Firewall Manager](https://docs.aws.amazon.com/waf/latest/developerguide/fms-chapter.html)
- [Amazon GuardDuty](https://docs.aws.amazon.com/guardduty/)
- [AWS IAM Identity Center](https://docs.aws.amazon.com/singlesignon/)
- [Amazon Inspector](https://docs.aws.amazon.com/inspector/)
- [AWS Key Management Service (AWS KMS)](https://docs.aws.amazon.com/kms/)
- [Amazon Macie](https://docs.aws.amazon.com/macie/)
- [AWS Network Firewall](https://docs.aws.amazon.com/network-firewall/)
- [AWS Resource Access Manager (AWS RAM)](https://docs.aws.amazon.com/ram/)
- [AWS Secrets Manager](https://docs.aws.amazon.com/secretsmanager/)
- [AWS Security Hub](https://docs.aws.amazon.com/securityhub/)
- [AWS Shield](https://docs.aws.amazon.com/shield/)
- [AWS WAF](https://docs.aws.amazon.com/waf/)
- [AWS Identity and Access Management (IAM)](https://docs.aws.amazon.com/IAM/)

---

# Sin servidor

- [AWS AppSync](https://docs.aws.amazon.com/appsync/)
- [AWS Fargate](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/AWS_Fargate.html)
- [AWS Lambda](https://docs.aws.amazon.com/lambda/)

---

# Almacenamiento

- [AWS Backup](https://docs.aws.amazon.com/aws-backup/)
- [Amazon EBS](https://docs.aws.amazon.com/ebs/)
- [Amazon EFS](https://docs.aws.amazon.com/efs/)
- [Amazon FSx](https://docs.aws.amazon.com/fsx/)
- [Amazon S3](https://docs.aws.amazon.com/s3/)
- [Amazon S3 Glacier](https://docs.aws.amazon.com/amazonglacier/)
- [AWS Storage Gateway](https://docs.aws.amazon.com/storagegateway/)

---

# Recursos oficiales del examen

- [AWS Certification](https://aws.amazon.com/certification/)
- [AWS Certified Solutions Architect – Associate](https://aws.amazon.com/certification/certified-solutions-architect-associate/)
- [Guía oficial del examen SAA-C03](https://d1.awsstatic.com/training-and-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Exam-Guide.pdf)
- [AWS Skill Builder](https://explore.skillbuilder.aws/)
- [AWS Well-Architected Framework](https://docs.aws.amazon.com/wellarchitected/)
- [AWS Architecture Center](https://aws.amazon.com/architecture/)
- [AWS Whitepapers](https://docs.aws.amazon.com/whitepapers/)

---

# Recomendación de estudio

Para preparar el examen SAA-C03, prioriza estos grupos:

## Alta prioridad

- IAM
- Amazon VPC
- Amazon S3
- Amazon EC2
- Elastic Load Balancing
- Auto Scaling
- Amazon RDS
- Amazon Aurora
- Amazon DynamoDB
- AWS Lambda
- Amazon CloudFront
- Route 53
- AWS Organizations
- AWS KMS
- AWS WAF
- AWS Backup
- SQS, SNS y EventBridge

## Prioridad media

- ECS
- EKS
- Fargate
- Step Functions
- API Gateway
- ElastiCache
- Storage Gateway
- DataSync
- Direct Connect
- Transit Gateway
- Secrets Manager
- GuardDuty
- CloudWatch
- CloudTrail
- Config

## Prioridad complementaria

- Servicios de Machine Learning
- Servicios multimedia
- AWS Device Farm
- AWS Data Exchange
- Amazon Kendra
- Amazon Lex
- Amazon Polly
- Amazon Rekognition

---

# Objetivo

Este archivo sirve como índice maestro de servicios dentro del alcance del examen SAA-C03.  
Úsalo junto con los documentos de los dominios 1, 2, 3 y 4 para construir una guía completa de preparación.

---

**Navegación:** [Inicio](../README.md) · [Anterior: Preguntas - Dominio 4](../SAA-C03-Questions/SAA-C03-Questions-D4.md)

